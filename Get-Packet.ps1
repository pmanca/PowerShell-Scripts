...
#
# Peter Manca 7/24/2015
#
#This is a test version of the Get-Packet Script  
...
################################## You can change the $countTotal for how many packets you want to get also you can change the output path as well
$Statistics = $true
$countTotal = 100
$count = 0
$Outputfile = "C:\test\packets.txt"
...
while ($running)
	{
$count++
if($count -gt $countTotal){$running = $false}
...
if ($Statistics)
    ...
    Write-Host $stats.DestinationPorts
    $stats.DestinationPorts | Format-Table | Out-File -FilePath "C:\test\DestPorts.txt"
    Write-Host $stats.Destinations
    $stats.Destinations | Format-Table | Out-File -FilePath "C:\test\Dests.txt"
    #$stats | Format-Table |Out-File -FilePath "C:\test\statistics.txt"
     $stats | ConvertTo-Html | Out-File -FilePath "C:\test\statistics.txt"
